...
class GDRN(nn.Module):
    def __init__(self, cfg, backbones, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "GDRN_stereo_early_mono_no_weight_sharing", cfg.MODEL.POSE_NET.NAME
        self.backbone_l = backbones[0]
        self.backbone_r = backbones[1]
        self.neck = neck
    ...
    def forward(
        ...
        # conv_feat_l = self.backbone_l(x.reshape((bs_virtual, c, h, w)))  # [bs_virtual, c, 8, 8]
        conv_feat_l = self.backbone_l(x[:, 0])
        conv_feat_r = self.backbone_r(x[:, 1])
        if type(conv_feat_l) == list:
            conv_feat_l = conv_feat_l[0]
        if type(conv_feat_r) == list:
            conv_feat_r = conv_feat_r[0]

        channel_half = conv_feat_l.shape[-3] // 2
        ...
        # conv_feat = torch.cat(
            ...
            # )
        conv_feat = torch.cat(
            (conv_feat_l[:, :channel_half, ::], conv_feat_r[:, :channel_half, ::]),
            dim=-3
        )
...
def build_model_optimizer(cfg, is_test=False):
    ...
    backbones = (
        BACKBONES[backbone_type](**init_backbone_args),
        BACKBONES[backbone_type](**init_backbone_args)
    )
    ...
    # build model
    model = GDRN(cfg, backbones, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net)
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
                "params": filter(

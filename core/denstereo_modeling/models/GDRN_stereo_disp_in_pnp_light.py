...
class GDRN(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, selfocc_head_net, neck=None, pnp_net=None, disp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "GDRN_stereo_disp_in_pnp_light", cfg.MODEL.POSE_NET.NAME
    ...
    def forward(
        ...
        if not do_loss:  # test
            ...
            # axs[0,0].imshow(gt_disp[0,0].cpu().detach(), vmin=0, vmax=64)
            # axs[1,0].imshow(disps[3][0].cpu().detach(), vmin=0, vmax=64)
            # axs[2,0].imshow(gt_mask_visib[0,0].cpu().detach())
            # axs[3,0].imshow(x[0,0].cpu().detach().numpy().transpose((1,2,0)))
            # axs[0,1].imshow((gt_mask_visib[0,0] * gt_disp[0,0]).cpu().detach(), vmin=0, vmax=64)
            # axs[1,1].imshow((gt_mask_visib[0,0] * disps[3][0]).cpu().detach(), vmin=0, vmax=64)
            # axs[2,1].imshow(gt_mask_visib[0,0].cpu().detach())
            # axs[3,1].imshow((gt_mask_visib[0,0] * x[0,0]).cpu().detach().numpy().transpose((1,2,0)))
            # plt.show()
    ...
    def gdrn_loss(
        ...
        if not disp_net_cfg.FREEZE and loss_cfg.DISP_LW > 0.0:
            loss_dict["loss_disp"] = msn_loss(out_disp[0], gt_disp[:, 0], gt_mask_visib[:, 0]) # left

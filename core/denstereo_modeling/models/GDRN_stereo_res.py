...
class GDRN(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, selfocc_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert cfg.MODEL.POSE_NET.NAME == "GDRN_stereo_res", cfg.MODEL.POSE_NET.NAME
    ...
    def forward(
        ...
        residual, conv_feat = self.backbone(x.reshape((bs_virtual, c, h, w)))  # [bs_virtual, c, 8, 8]
        ...
        # give residual tensor from third layer of resnet to pnp_net
        if pnp_net_cfg.WITH_RESIDUAL:
            assert residual is not None
            _, c, w, h = residual.shape
            coor_feat = torch.cat([coor_feat, residual.view((bs, 2, c, w, h))], dim=-3)

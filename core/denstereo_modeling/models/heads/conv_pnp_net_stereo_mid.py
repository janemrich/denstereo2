...
class ConvPnPNetStereoMid(nn.Module):
    def __init__(
        self,
        nIn,
        num_regions=8,
        mask_attention_type="none",
        featdim=256, # from 128
        rot_dim=6,
        num_stride2_layers=3,
        num_extra_layers=2, # from 0
        norm="GN",
        num_gn_groups=32,
        act="relu",
        drop_prob=0.0,
        ...
        for i in range(num_stride2_layers):
            _in_channels = nIn*2 if i == 0 else featdim*2 # * 2 for stereo
            self.features.append(nn.Conv2d(_in_channels, featdim*2, kernel_size=3, stride=2, padding=1, bias=False)) # *2 stereo
            self.features.append(get_norm(norm, featdim*2, num_gn_groups=num_gn_groups)) # *2 stereo
            self.features.append(conv_act)

        for i in range(num_extra_layers):
            _in_channels = featdim*2 if i == 0 else featdim # * 2 for stereo
            self.features.append(nn.Conv2d(_in_channels, featdim, kernel_size=3, stride=1, padding=1, bias=False))
    ...
    def forward(
        ...
        """
        s, bs, in_c, fh, fw = coor_feat.shape
        '''
        if in_c == 3 or in_c == 5:
        ...
        if self.mask_attention_type != "none":
            ...
            if self.mask_attention_type == "mul":
                ...
                x = torch.cat([x, mask_attention], dim=-3)
            else:
                raise ValueError(f"Wrong mask attention type: {self.mask_attention_type}")

        x = torch.cat((x[:, 1, ::], x[:, 0, ::]), dim=-3)

        if self.drop_prob > 0:
            self.dropblock.step()  # increment number of iterations
            x = self.dropblock(x)
            self.dropblock.step()
            x_right = self.dropblock(x_right)

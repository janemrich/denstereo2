...
from .MSNet2D_submodules import feature_extraction, MobileV2_Residual, convbn, interweave_tensors, disparity_regression
...
class MSNet2Dlight(nn.Module):
    def __init__(self, maxdisp):

        super(MSNet2Dlight, self).__init__()
        ...
        self.preconv11 = nn.Sequential(convbn(512, 256, 1, 1, 0, 1), # lightstereo change
                                       nn.ReLU(inplace=True),
                                       convbn(256, 128, 1, 1, 0, 1),
                                       nn.ReLU(inplace=True),
                                       convbn(128, 64, 1, 1, 0, 1),
    ...
    def forward(self, L, R):
        ...
        featL = self.preconv11(L)
        featR = self.preconv11(R)
        ...
        if self.training:
            ...
            cost0 = F.interpolate(cost0, [self.maxdisp, L.size()[2]*2, L.size()[3]*2], mode='trilinear') # lightstereo
            cost0 = F.interpolate(cost0, [self.maxdisp, cost0.size()[-2], cost0.size()[-1]], mode='trilinear')
            cost0 = torch.squeeze(cost0, 1)
            pred0 = F.softmax(cost0, dim=1)
            pred0 = disparity_regression(pred0, self.maxdisp)

            cost1 = torch.unsqueeze(cost1, 1)
            cost1 = F.interpolate(cost1, [self.maxdisp, L.size()[2]*2, L.size()[3]*2], mode='trilinear') # lightstereo
            cost1 = F.interpolate(cost1, [self.maxdisp, cost1.size()[-2], cost1.size()[-1]], mode='trilinear')
            cost1 = torch.squeeze(cost1, 1)
            pred1 = F.softmax(cost1, dim=1)
            pred1 = disparity_regression(pred1, self.maxdisp)

            cost2 = torch.unsqueeze(cost2, 1)
            cost2 = F.interpolate(cost2, [self.maxdisp, L.size()[2]*2, L.size()[3]*2], mode='trilinear') # lightstereo
            cost2 = F.interpolate(cost2, [self.maxdisp, cost2.size()[-2], cost2.size()[-1]], mode='trilinear')
            cost2 = torch.squeeze(cost2, 1)
            pred2 = F.softmax(cost2, dim=1)
            pred2 = disparity_regression(pred2, self.maxdisp)

            cost3 = torch.unsqueeze(cost3, 1)
            cost3 = F.interpolate(cost3, [self.maxdisp, L.size()[2]*2, L.size()[3]*2], mode='trilinear') # lightstereo
            cost3 = F.interpolate(cost3, [self.maxdisp, cost3.size()[-2], cost3.size()[-1]], mode='trilinear')
